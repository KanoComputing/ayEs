...
const Auth = function Auth(secret, pub, options) {
	this.pub = pub;
	...
	this.pub = pub || false;
	if (!this.pub) {
		const error = new Error(null, 'JWT Public key not supplied.');
	...
	this.generateAuthMiddleWare = function generateAuthMiddleWare() {
		return function amw(req, res, next) {
			...
			try {
				dtoken = Auth.decodeJWT(token, this.pub);
			} catch (e) {
...
Auth.decodeJWT = function decodeJWT(jwt, pub) {
	return JWT.verify(jwt, pub, { algorithm: 'ES256' });
};

Auth.encodeJWT = function encodeJWT(payload, secret) {
	return JWT.sign(payload, secret, { algorithm: 'ES256' });
};
