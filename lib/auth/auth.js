...
const Auth = function Auth(secretOrPublicKey, options = {}) {
	...
	if (secretOrPublicKey.indexOf('BEGIN PUBLIC KEY')) {
...
Auth.decodeJWT = function decodeJWT(jwt, secretOrPublicKey) {
	return JWT.verify(jwt, secretOrPublicKey, { algorithm: Auth.algorithm });
};

Auth.encodeJWT = function encodeJWT(payload, secret) {
	return JWT.sign(payload, secret, { algorithm: Auth.algorithm });
};
